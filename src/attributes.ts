...
export function HasMany(target: () => Function, options: RelationOptions = {}) {
  return function (targetPrototype: Object, propertyKey: string | symbol) {
    const existingMetadata: Record<string, RelationMeta<() => Function>> =
      Reflect.getMetadata(RELATION_METADATA_KEY, targetPrototype.constructor) || {};
...
function getRelationMetaFromSelector<T>(
  ...
  const all: Record<string, RelationMeta<() => Function>> =
