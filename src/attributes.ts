...
export function Table(options: TableOptions = { name: null! }) {
  return function (target: Function) {
    ...
    Reflect.defineMetadata(TABLE_METADATA_KEY, { name: tableName, className } as TableMeta, target)
  }
}
...
export function getColumnsMetadata(target: Function): Record<string, ColumnMeta> {
  return Reflect.getMetadata(COLUMN_METADATA_KEY, target) || {}
}

export function getRelationsMetadata<T extends () => Function>(target: Function): Record<string, RelationMeta<T>> {
  return Reflect.getMetadata(RELATION_METADATA_KEY, target) || []
}

interface RelationOptions {
  cascade?: boolean | Array<'insert' | 'update' | 'remove' | 'soft-remove' | 'recover'>
  eager?: boolean
...
export function HasMany(target: () => Function, options: RelationOptions = {}) {
  return function (targetPrototype: Object, propertyKey: string | symbol) {
    const existingMetadata: Record<string, RelationMeta<() => Function>> = Reflect.getMetadata(
    ...
    Reflect.defineMetadata(RELATION_METADATA_KEY, existingMetadata, targetPrototype.constructor)
  }
}
...
export function Column(options: ColumnOptions = { name: null! }) {
  return function (targetPrototype: Object, propertyKey: string | symbol) {
    const existingMetadata: Record<string, ColumnMeta> =
      Reflect.getMetadata(COLUMN_METADATA_KEY, targetPrototype.constructor) || {}
    ...
    Reflect.defineMetadata(COLUMN_METADATA_KEY, existingMetadata, targetPrototype.constructor)
  }
}
...
function extractPropertyNameProxy<T>(fn: (obj: T) => any): string {
  ...
  if (!match) throw new Error('Only arrow functions like "x => x.prop" are supported')
  return match[1]
}
...
function getRelationMetaFromSelector<T>(
  ...
  const all: Record<string, RelationMeta<() => Function>> = Reflect.getMetadata(RELATION_METADATA_KEY, model) || {}
  return all[field]
}
